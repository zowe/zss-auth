...
const AuthManager = require("zlux-server-framework/lib/auth-manager.js");
const log = global.COM_RS_COMMON_LOGGER.makeComponentLogger("_unp.auth.zss");

const BYPASS_SAF_CHECK = false;
const DEFAULT_CLASS = "XFACILIT"; //ZOWE";
...
ZssAuthenticator.prototype = {
    ...
    request.zluxData.webApp.callRootService("login", { 
      ...
      body: request.body
        ...
        if (zssCookie) {
          ...
          //console.log("authenticate ok: sessionState", sessionState)
  ...
  authorized: Promise.coroutine(function *authorized(request, sessionState, 
    ...
    try {
      ...
      const resourceName = this._makeResourceName(request.originalUrl, 
          request.method);
      if (syncOnly) {
      ...
      this._processAgentResponse(httpResponse, result);
  ...
  _makeResourceName(originalUrl, method) {
    //console.log("request.originalUrl", request.originalUrl)
    const originalPath = url.parse(originalUrl).pathname;
    //console.log("originalPath", originalPath)
    const resourceName = encodeURIComponent(AuthManager.getResourceName(
        originalPath, method));
    return resourceName;
  },
